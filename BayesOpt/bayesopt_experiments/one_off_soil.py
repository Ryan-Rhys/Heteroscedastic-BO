...
if __name__ == '__main__':
    ...
    fill = True
    penalty = 20  # penalty weighting for noise
    aleatoric_weight = 100  # weighting for aleatoric noise in the acquisition functions
    ...
    for i in range(random_trials):
        ...
        xs_train, xs_test, ys_train, ys_test = train_test_split(xs, ys, test_size=0.4, shuffle=True, random_state=numpy_seed)  # use (xs_test, ys_test) to initialise
        ...
        for i in range(bayes_opt_iters):
            ...
            aug_het_composite_obj_val = aug_het_Y_next - penalty*std[aug_het_index]
        ...
        rand_running_sum += np.array(rand_obj_val_list, dtype=np.float64).flatten()  # just the way to average out across all random trials
        rand_squares += np.array(rand_obj_val_list, dtype=np.float64).flatten() ** 2  # likewise for errors
    ...
    plt.legend(loc=1)
    plt.savefig('one_off_soil_figures/bayesopt_plot{}_iters_{}_random_trials_and_init_num_samples_of_{}_and_seed_{}_new_acq_penalty_is_{}'.
