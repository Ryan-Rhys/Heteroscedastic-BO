...
from objective_functions import max_quasar_objective
from utils import load_quasar, get_y_and_std_from_x, plot_BO_progress, get_x_idx
from bo_gp_fit_predict import bo_fit_homo_gp, bo_predict_homo_gp, bo_fit_hetero_gp, bo_predict_hetero_gp


if __name__ == '__main__':

    X_ref, Y_ref, U_ref = load_quasar()
    ...
    for i in range(random_trials):
        ...
        for j in range(bayes_opt_iters):
            ...
            homo_Y_next, _ = get_y_and_std_from_x(homo_X_next,load_quasar)
            homo_composite_obj_val = max_quasar_objective(homo_X_next)
            ...
            # Obtain next noisy sample from the objective function
            het_Y_next, _ = get_y_and_std_from_x(het_X_next,load_quasar)
            het_composite_obj_val = max_quasar_objective(het_X_next)
    ...
    plot_BO_progress(my_homo_obj[:i+1].mean(0),my_homo_obj[:i+1].std(0),
                    my_het_obj[:i+1].mean(0),my_het_obj[:i+1].std(0),filepath='quasar_BO.pdf')
