...
"""
This script plots samples from a GP prior.
"""
...
if __name__ == "__main__":
    ...
    one_dimension = False  # Plot GP fit in one dimension
    two_dimensions = True  # Plot GP fit in two dimensions on the Branin function.
    two_dims_with_samples = False  # Plot GP fit in two dimensions on the Branin function with samples from the posterior.
    ...
    if one_dimension:
        ...
        # Compute the GP posterior (generalisation of Rasmussen and Williams algorithm pg. 19)

        noise = 0.2
        xs_star = np.arange(-5, 5, 0.2).reshape(-1, 1).reshape(50,)  # test locations

        # We optimise the GP hyperparameters by minimizing the negative log marginal likelihood.
